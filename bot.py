import logging, os, uuid, shutil, requests, asyncio, subprocess, sys, threading, re, glob
...
def resolve_metadata_from_link(url):
    ...
    try:
        ...
        logger.error(f"Metadata parse failed for {url}: {e}")
...
async def execute_deezer_search(query, temp_dir):
    try:
        ...
        if not data.get("data"):
            logger.warning(f"No Deezer data for query: {query}")
            return None
        ...
        cmd = [
            'ytmdl', '--quiet', '--format', 'm4a', '--skip-meta', '--output-dir', temp_dir, search_q
        ]
        res = await run_async(subprocess_run_with_timeout, cmd, 180)

        if res.returncode != 0:
            logger.error(f"ytmdl failed for {search_q}: {res.stderr[-200:]}")
            return None

        # Find the downloaded file (ytmdl saves as "Title.m4a" or similar)
        files = glob.glob(os.path.join(temp_dir, "*.m4a"))
        if not files:
            logger.error(f"No m4a file found after ytmdl for {search_q}")
            return None
        audio_path = files[0]  # Take the first one
...
async def merge_and_send(update: Update, context: ContextTypes.DEFAULT_TYPE, pending: dict, status_msg=None):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Upload document failed: {e2}")
