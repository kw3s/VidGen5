...
MAX_WORKERS = 2
BASE_TEMP_DIR = "/data/temp" if os.path.exists("/data") else "temp"
...
async def execute_deezer_search(query, temp_dir):
    try:
        ...
        search_q = f"{artist} {title} official audio"
        cmd = ['yt-dlp', '-x', '--audio-format', 'm4a', '-o', audio_path, f'ytsearch1:{search_q}', '--quiet', '--no-playlist',
               '--format', 'bestaudio[ext=m4a]/bestaudio', '--audio-quality', '5', '--retries', '3', '--fragment-retries', '3',
               '--sleep-interval', '5', '--max-sleep-interval', '15', '--force-ipv4']
        ...
        return {"audio": audio_path, "image": image_path, "caption": caption}
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msgs = {
        MODE_LINKS: "🔗 **Link Mode**\nSend a link (Spotify, Tidal, Deezer).",
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.text and not (update.message.photo or update.message.audio or update.message.voice):
        ...
        if mode == MODE_LINKS:
            ...
            result = await execute_deezer_search(meta_query, context.user_data["temp_dir"])
            if not result:
                await status_msg.edit_text("❌ Failed to find or download audio.")
                context.user_data["processing"] = False
                return
            ...
            result = await execute_deezer_search(text, context.user_data["temp_dir"])
            if not result:
...
# --- UPLOAD & RENDER (OPTIMIZED FURTHER) ---
async def merge_and_send(update: Update, context: ContextTypes.DEFAULT_TYPE, pending: dict, status_msg=None):
    ...
    # Optimized FFmpeg: low framerate for static image, CRF for fast compression, copy audio if possible, ultrafast preset for low resources
    cmd = [
        FFMPEG, '-threads', '1', '-loop', '1', '-framerate', '2', '-i', pending["image"], '-i', pending["audio"],
        '-c:v', 'libx264', '-tune', 'stillimage', '-preset', 'ultrafast', '-crf', '28',
        '-c:a', 'copy',  # Faster: copy audio if compatible (AAC/m4a inputs work)
        '-shortest', '-movflags', '+faststart', '-vf', 'scale=480:-2,format=yuv420p,pad=ceil(iw/2)*2:ceil(ih/2)*2', '-y', output_path
    ]
